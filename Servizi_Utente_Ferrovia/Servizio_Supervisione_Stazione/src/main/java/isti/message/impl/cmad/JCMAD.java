...
@NamedQueries({
	...
	query="SELECT c FROM Jcmad c WHERE c.Id.MAC_ADR= ?1 ORDER BY c.Id.DATE DESC"),
	@NamedQuery(name="JCMAD.findMacBetweenTime",
	query="SELECT c FROM Jcmad c WHERE c.Id.MAC_ADR= ?1  and c.Id.DATE BETWEEN ?2 AND ?3 ORDER BY c.Id.DATE DESC"),
	@NamedQuery(name="JCMAD.findAllorder",
	query="SELECT c FROM Jcmad c ORDER BY c.Id.DATE DESC"),
